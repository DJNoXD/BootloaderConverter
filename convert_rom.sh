...
while true; do
	...
	echo "========================================"
	echo
	#echo "Converts a ROM to work on the other bootloader."
	echo "Converts a ROM for the other bootloader."
	echo
	#echo "Select the convertion direction"
	echo "Select the desired convertion:"
	echo "----------------------------------------"
	echo  "   1 - Convert ROM for OLD BootLoader"
	echo  "   2 - Convert ROM for NEW BootLoader"
	echo 
	echo  "   X - Cancel"                        
	echo
	read -n1 -p "Please type your selection [1,2,X]: " action
	case $action in
		1) prefixto=oldbl; prefixfrom=newbl; break ;;
		2) prefixto=newbl; prefixfrom=oldbl; break ;;
		[Xx]*) exit ;; 
		*) echo;echo "!! Wrong selection !!" ;; 
	esac
done
echo
...
if [ "$(ls -A $srczips)" ]; then
	SelectFile "$srczips/*.zip" "Select the desired ROM to convert" "ROM"
	romzip=$(sed -n "${answr}{p;q;}" list.txt) #read the nth specific line 
	rm list.txt
else
